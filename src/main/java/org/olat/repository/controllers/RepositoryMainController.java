 ...
 * 
 * Activatable:<br>
 * Supported commands are: "search.home": root entry point "search.catalog":
 * catalog view "search.generic": search form "search.my": list of owned
 * resources "search.REPOTYPE": list of all resources of given type, where
 * REPOTYPE is something like course, test, survey, cp, scorm, sharedfolder
 * 
 * @date Initial Date: Oct 21, 2004 <br>
 * @author Felix Jost
 */
public class RepositoryMainController extends MainLayoutBasicController
		implements Activateable2 {

	private static final OLog log = Tracing
			.createLoggerFor(RepositoryMainController.class);
	private static final String VELOCITY_ROOT = Util
			.getPackageVelocityRoot(RepositoryManager.class);
	...
	public RepositoryMainController(UserRequest ureq, WindowControl wControl) {
		...
		if (log.isDebug()) {
			log.debug("Constructing ReposityMainController for user::"
					+ ureq.getIdentity());
		}
		businessGroupService = CoreSpringFactory
				.getImpl(BusinessGroupService.class);
		portfolioModule = (PortfolioModule) CoreSpringFactory
				.getBean("portfolioModule");
		...
		main = new VelocityContainer("vc_index", RepositoryManager.class,
				"index", getTranslator(), this);

		isAuthor = ureq.getUserSession().getRoles().isAuthor();
		// if the user has the role InstitutionalResourceManager (and has the
		// same
		...
		searchController = new RepositorySearchController(
				translate("details.header"), ureq, getWindowControl(), false,
				true, false);
		listenTo(searchController);
		main.put("searchcomp", searchController.getInitialComponent());

		detailsController = new RepositoryDetailsController(ureq,
				getWindowControl());
		...
		createRepoToolController(isAuthor, ureq.getUserSession().getRoles()
				.isOLATAdmin());
		...
		Component toolComp = (mainToolC == null ? null : mainToolC
				.getInitialComponent());
		columnsLayoutCtr = new LayoutMain3ColsController(ureq,
				getWindowControl(), menuTree, toolComp, mainPanel, "repomain");
		...
		if (isAuthor) {
			...
			if(isCourseEnabled()){
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_COURSE,
					translate("tools.add.course"),
					RepositoryAddController.ACTION_ADD_COURSE,
					"o_toolbox_course o_sel_repo_import_course");
			}
			if (isCPEnabled()) {
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_CP,
						translate("tools.add.cp"),
						RepositoryAddController.ACTION_ADD_CP,
						"o_toolbox_content o_sel_repo_import_cp");
			}
			if (isSCORMEnabled()) {
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_SCORM,
						translate("tools.add.scorm"),
						RepositoryAddController.ACTION_ADD_SCORM,
						"o_toolbox_scorm o_sel_repo_import_scorm");
			}
			if (isWikiEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_WIKI,
						translate("tools.add.wiki"),
						RepositoryAddController.ACTION_ADD_WIKI,
						"o_toolbox_wiki o_sel_repo_import_wiki");
			if (isPodcastEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_PODCAST,
						translate("tools.add.podcast"),
						RepositoryAddController.ACTION_ADD_PODCAST,
						"o_toolbox_podcast o_sel_repo_import_podcast");
			if (isBlogsEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_BLOG,
						translate("tools.add.blog"),
						RepositoryAddController.ACTION_ADD_BLOG,
						"o_toolbox_blog o_sel_repo_import_blog");
			if (isTestEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_TEST,
						translate("tools.add.test"),
						RepositoryAddController.ACTION_ADD_TEST,
						"o_toolbox_test o_sel_repo_import_test");
			if (isQuasitionariesEnabled())
				mainToolC
						.addLink(RepositoryAddController.ACTION_ADD_SURVEY,
								translate("tools.add.survey"),
								RepositoryAddController.ACTION_ADD_SURVEY,
								"o_toolbox_questionnaire o_sel_repo_import_questionnaire");
			if (isGlossaryEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_GLOSSARY,
						translate("tools.add.glossary"),
						RepositoryAddController.ACTION_ADD_GLOSSARY,
						"o_toolbox_glossary o_sel_repo_import_glossary");
			if (isResourceFolderEnabled())
				mainToolC.addLink(RepositoryAddController.ACTION_ADD_DOC,
			...
			if (isCourseEnabled()) {
				mainToolC.addLink(ACTION_NEW_CREATECOURSE,
						translate("tools.new.createcourse"),
						ACTION_NEW_CREATECOURSE,
						"o_toolbox_course o_sel_repo_new_course");
			}
			if (isCPEnabled()) {
				mainToolC.addLink(ACTION_NEW_CREATECP,
						translate("tools.new.createcp"), ACTION_NEW_CREATECP,
						"o_toolbox_content o_sel_repo_new_cp");
			}
			if (isWikiEnabled()) {
				mainToolC.addLink(ACTION_NEW_WIKI, translate("tools.new.wiki"),
						ACTION_NEW_WIKI, "o_toolbox_wiki o_sel_repo_new_wiki");
			}
			if (isPodcastEnabled()) {
				mainToolC.addLink(ACTION_NEW_PODCAST,
						translate("tools.new.podcast"), ACTION_NEW_PODCAST,
						"o_toolbox_podcast o_sel_repo_new_podcast");
			}
			if (isBlogsEnabled()) {
				mainToolC.addLink(ACTION_NEW_BLOG, translate("tools.new.blog"),
						ACTION_NEW_BLOG, "o_toolbox_blog o_sel_repo_new_blog");
			}
			if (portfolioModule.isEnabled() && isPortfolioTemplateEnabled()) {
				mainToolC.addLink(ACTION_NEW_PORTFOLIO,
						translate("tools.new.portfolio"), ACTION_NEW_PORTFOLIO,
						"o_toolbox_portfolio o_sel_repo_new_portfolio");
			}
			if (isTestEnabled()) {
				mainToolC.addLink(ACTION_NEW_CREATETEST,
						translate("tools.new.createtest"),
						ACTION_NEW_CREATETEST,
						"o_toolbox_test o_sel_repo_new_test");
			}
			if (isQuasitionariesEnabled()) {
				mainToolC.addLink(ACTION_NEW_CREATESURVEY,
						translate("tools.new.createsurvey"),
						ACTION_NEW_CREATESURVEY,
						"o_toolbox_questionnaire o_sel_repo_new_questionnaire");
			}
			if (isResourceFolderEnabled()) {
				mainToolC.addLink(ACTION_NEW_CREATESHAREDFOLDER,
						translate("tools.new.createsharedfolder"),
						ACTION_NEW_CREATESHAREDFOLDER,
						"o_toolbox_sharedfolder o_sel_repo_new_sharedfolder");
			}
			if (isGlossaryEnabled()) {
				mainToolC.addLink(ACTION_NEW_GLOSSARY,
						translate("tools.new.glossary"), ACTION_NEW_GLOSSARY,
						"o_toolbox_glossary o_sel_repo_new_glossary");
			}
			if (bIsAdmin || isAuthor) {
				mainToolC.addHeader(translate("tools.administration.header"));
				if (bIsAdmin) {
					mainToolC.addLink(ACTION_DELETE_RESOURCE,
							translate("tools.delete.resource"));
				}
				mainToolC.addLink(ACTION_ADD_OWNERS,
						translate("tools.add.owners"));
			}
		} else {
			mainToolC = null;
		}
	...
	private TreeModel buildTreeModel(boolean bIsAuthor) {
		...
		GenericTreeNode rootNode = new GenericTreeNode(
				translate("search.home"), "search.home");
		...
		boolean repoPortletOn = PortletFactory
				.containsPortlet("RepositoryPortletStudent");
		// add default searches
		node = new GenericTreeNode(translate("search.generic"),
				"search.generic");
		node.setCssClass("o_sel_repo_search_generic");
		rootNode.addChild(node);
		if (bIsAuthor) {
			GenericTreeNode myEntriesTn = new GenericTreeNode(
					translate("search.my"), "search.my");
		...
		if (repoPortletOn) {
			if(isCourseEnabled()){
			node = new GenericTreeNode(translate("search.mycourses.student"),
					"search.mycourses.student");
			node.setCssClass("o_sel_repo_my_student");
			rootNode.addChild(node);
			}
			// for authors or users with group rights also show the teacher
			// portlet
			if (bIsAuthor
					|| RepositoryManager.getInstance()
							.countLearningResourcesAsTeacher(getIdentity()) > 0) {
				if(isCourseEnabled()){
				node = new GenericTreeNode(
						translate("search.mycourses.teacher"),
						"search.mycourses.teacher");
		...
		if (bIsAuthor) {
			// cp, scorm, wiki, podcast, portfolie, test, questionn, resource
			// folder, glossary
			if (isCPEnabled()) {
			...
			if (isSCORMEnabled()) {
				node = new GenericTreeNode(translate("search.scorm"),
						"search.scorm");
				node.setCssClass("o_sel_repo_scorm");
				rootNode.addChild(node);
			}
			if (isWikiEnabled()) {
				node = new GenericTreeNode(translate("search.wiki"),
						"search.wiki");
				node.setCssClass("o_sel_repo_wiki");
				rootNode.addChild(node);
			}
			if (isPodcastEnabled()) {
				node = new GenericTreeNode(translate("search.podcast"),
						"search.podcast");
				node.setCssClass("o_sel_repo_podcast");
				rootNode.addChild(node);
			}
			if (isBlogsEnabled()) {
				node = new GenericTreeNode(translate("search.blog"),
						"search.blog");
			...
			if (isTestEnabled()) {
				node = new GenericTreeNode(translate("search.test"),
						"search.test");
				node.setCssClass("o_sel_repo_test");
				rootNode.addChild(node);
			}
			if (isQuasitionariesEnabled()) {
				node = new GenericTreeNode(translate("search.survey"),
						"search.survey");
				node.setCssClass("o_sel_repo_survey");
				rootNode.addChild(node);
			}
			if (isResourceFolderEnabled()) {
				node = new GenericTreeNode(translate("search.sharedfolder"),
						"search.sharedfolder");
				node.setCssClass("o_sel_repo_sharefolder");
				rootNode.addChild(node);
			}
			if (isGlossaryEnabled()) {
				node = new GenericTreeNode(translate("search.glossary"),
						"search.glossary");
	...
	public void event(UserRequest ureq, Component source, Event event) {
		long start = System.currentTimeMillis(); // TODO: add check if debug
													// logging
		if (source == menuTree) {
			if (event.getCommand().equals(MenuTree.COMMAND_TREENODE_CLICKED)) {
				...
				TreeNode clickedNode = menuTree.getTreeModel().getNodeById(
						te.getNodeId());
	...
	private void activateContent(UserRequest ureq, Object userObject,
			List<ContextEntry> entries, StateEntry state) {
		...
		if (userObject.equals("search.home")) { // the
			...
			main.setPage(VELOCITY_ROOT + "/index2.html");
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					CourseModule.getCourseTypeName(), ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					TestFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					SurveyFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					ImsCPFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					ScormCPFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					SharedFolderFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
		} else if (userObject.equals("search.wiki")) { // search
			// wiki
			main.setPage(VELOCITY_ROOT + "/index2.html");
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(WikiResource.TYPE_NAME,
					ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					PodcastFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					BlogFileResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					GlossaryResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
			...
			mainPanel.setContent(main);
			searchController.doSearchByTypeLimitAccess(
					EPTemplateMapResource.TYPE_NAME, ureq);
			searchController.enableBackToSearchFormLink(false);
	...
	private void activateCatalogController(UserRequest ureq,
			List<ContextEntry> entries, StateEntry state) {
		...
		if (catalogCntrllr == null || lastUserObject.equals("search.catalog")) {
			...
			OLATResourceable ores = OresHelper.createOLATResourceableInstance(
					"search.catalog", 0l);
			WindowControl bwControl = BusinessControlFactory
					.getInstance()
		...
		Component toolComp = (ccToolCtr == null ? null : ccToolCtr
				.getInitialComponent());
		columnsLayoutCtr.setCol2(toolComp);
	}
	...
	public void event(UserRequest urequest, Controller source, Event event) {
		if (source == chooseStepsController) {
			if (event
					.equals(RepositoryAddChooseStepsController.CREATION_WIZARD)) {
				cmc.deactivate();
				RepositoryEntry addedEntry = chooseStepsController
						.getCourseRepositoryEntry();
				RepositoryHandler handler = RepositoryHandlerFactory
						.getInstance().getRepositoryHandler(addedEntry);
				creationWizardController = handler.createWizardController(
						addedEntry, urequest, getWindowControl());
				listenTo(creationWizardController);
				getWindowControl().pushAsModalDialog(
						creationWizardController.getInitialComponent());
			} else if (event
					.equals(RepositoryAddChooseStepsController.DETAILS_VIEW)) {
				// show details view
				cmc.deactivate();
			} else if (event
					.equals(RepositoryAddChooseStepsController.COURSE_EDIT)) {
				// show course editor
				cmc.deactivate();
				ToolController toolC = detailsController.setEntry(
						chooseStepsController.getCourseRepositoryEntry(),
						urequest, false);
				Component toolComp = (toolC == null ? null : toolC
						.getInitialComponent());
				columnsLayoutCtr.setCol2(toolComp);
			...
			if (event.equals(Event.CHANGED_EVENT)) {
				...
				detailsController.setEntry(
						chooseStepsController.getCourseRepositoryEntry(),
						urequest, false);
				detailsController.doLaunch(urequest);
			} else if (event.equals(Event.CANCELLED_EVENT)) {
				// delete entry when the wizard was cancelled
				detailsController.deleteRepositoryEntry(urequest,
						getWindowControl(),
						(RepositoryEntry) chooseStepsController
								.getCourseRepositoryEntry());
				getWindowControl().pop();
				removeAsListenerAndDispose(creationWizardController);
				mainPanel.setContent(main);
			}
		} else if (source == addController) { // process add controller
												// finished()
			if (event.equals(Event.DONE_EVENT)) {
				// default to myEntries search
				cmc.deactivate();
				ToolController toolC = detailsController.setEntry(
						addController.getAddedEntry(), urequest, false);
				Component toolComp = (toolC == null ? null : toolC
						.getInitialComponent());
				mainPanel.setContent(detailsController.getInitialComponent());
				columnsLayoutCtr.setCol2(toolComp);
				...
				RepositoryHandler handler = RepositoryHandlerFactory
						.getInstance().getRepositoryHandler(
								addController.getAddedEntry());
				// ask if we should start the editor/wizard right now.
				if (handler.supportsEdit(addController.getAddedEntry())) {
					// check if wizard is supported and if the repo entry is new
					// (not imported)
					boolean isNew = addController.getActionProcess().equals(
							RepositoryAddController.PROCESS_NEW);
					if (handler.supportsWizard(addController.getAddedEntry())
							&& isNew) {
						removeAsListenerAndDispose(chooseStepsController);
						chooseStepsController = new RepositoryAddChooseStepsController(
								urequest, getWindowControl(),
								addController.getAddedEntry());
						listenTo(chooseStepsController);
						cmc = new CloseableModalController(getWindowControl(),
								translate("close"),
								chooseStepsController.getInitialComponent());
						cmc.activate();
					} else {
						removeAsListenerAndDispose(launchEditorDialog); // cleanup
																		// old
																		// instance
						launchEditorDialog = activateYesNoDialog(urequest,
								null, translate("add.launchedit.msg"),
								launchEditorDialog);
						launchEditorDialog.setUserObject(addController
								.getAddedEntry());
					}
			...
			// FXOLAT-398 searchControlle.getSelectedEntry can be null, catch
			// that to prevent NPE
			...
			RepositoryEntry selectedEntry = RepositoryManager.getInstance()
					.lookupRepositoryEntry(
							searchController.getSelectedEntry().getKey());
			...
			if (event.getCommand().equals(
					RepositoryTableModel.TABLE_ACTION_SELECT_ENTRY)) {
				// entry has been selected to be launched in the
				// searchController
				ToolController toolC = detailsController.setEntry(
						selectedEntry, urequest, false);
				if (selectedEntry.getCanLaunch()) {
					if (!detailsController.doLaunch(urequest)) {
						...
						Component toolComp = (toolC == null ? null : toolC
								.getInitialComponent());
						columnsLayoutCtr.setCol2(toolComp);
						mainPanel.setContent(detailsController
								.getInitialComponent());
					}
			...
			} else if (event.getCommand().equals(
				...
				mainPanel.setContent(detailsController.getInitialComponent());
				// fxdiff BAKS-7 Resume function
				addToHistory(urequest, selectedEntry, null,
						searchWindowControl, true);
			}
		} else if (source == detailsController) { // back from details
			// fxdiff FXOLAT-128: back/resume function
			if (event.equals(Event.DONE_EVENT)
					|| event.equals(RepositoryDetailsController.LAUNCHED_EVENT)) {
				if (backtocatalog) {
					backtocatalog = false;
					ToolController toolC = catalogCntrllr
					...
					// fxdiff BAKS-7 Resume function
					if (!event
							.equals(RepositoryDetailsController.LAUNCHED_EVENT)) {
				...
				ToolController toolC = detailsController
			...
			if (DialogBoxUIFactory.isYesEvent(event)) {
				...
				// editor
				ToolController toolC = detailsController.setEntry(
						(RepositoryEntry) launchEditorDialog.getUserObject(),
						urequest, false);
				Component toolComp = (toolC == null ? null : toolC
						.getInitialComponent());
				columnsLayoutCtr.setCol2(toolComp);
		 ...
		 */
			...
			if (event == Event.CHANGED_EVENT) {
				...
				// REVIEW:pb:the first event is also a change event, signalling
				// a change
				// in the tools
				// REVIEW:pb:the EntryChangedEvent is misused to signal
				// "show details"
				EntryChangedEvent entryChangedEvent = (EntryChangedEvent) event;
				RepositoryEntry e = RepositoryManager.getInstance()
						.lookupRepositoryEntry(
								entryChangedEvent.getChangedEntryKey());
				if (e != null) {
					backtocatalog = true;
					ToolController toolC = detailsController.setEntry(e,
							urequest, false);
					Component toolComp = (toolC == null ? null : toolC
							.getInitialComponent());
					columnsLayoutCtr.setCol2(toolComp);
		...
		} else if (source == cmc) { // user closes the overlay and not the
									// cancel button -> clean up repo entry
	...
	private void handleToolEvents(UserRequest urequest, Event event) {
		...
		if (event.getCommand().startsWith(
				RepositoryAddController.ACTION_ADD_PREFIX)) {
			removeAsListenerAndDispose(addController);
			addController = new RepositoryAddController(urequest,
					getWindowControl(), event.getCommand());
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			listenTo(cmc);
			cmc.activate();
			return;
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_COURSE);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			removeAsListenerAndDispose(addController);
			addController = new RepositoryAddController(urequest,
					getWindowControl(), RepositoryAddController.ACTION_NEW_TEST);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_SURVEY);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_SHAREDFOLDER);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			removeAsListenerAndDispose(addController);
			addController = new RepositoryAddController(urequest,
					getWindowControl(), RepositoryAddController.ACTION_NEW_WIKI);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_PODCAST);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			removeAsListenerAndDispose(addController);
			addController = new RepositoryAddController(urequest,
					getWindowControl(), RepositoryAddController.ACTION_NEW_BLOG);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_GLOSSARY);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			addController = new RepositoryAddController(urequest,
					getWindowControl(),
					RepositoryAddController.ACTION_NEW_PORTFOLIO);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			removeAsListenerAndDispose(deleteTabPaneCtr);
			deleteTabPaneCtr = new TabbedPaneController(urequest,
					getWindowControl());
			...
			removeAsListenerAndDispose(addController);
			addController = new RepositoryAddController(urequest,
					getWindowControl(), RepositoryAddController.ACTION_NEW_CP);
			listenTo(addController);
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), addController.getInitialComponent());
			...
			removeAsListenerAndDispose(cmc);
			cmc = new CloseableModalController(getWindowControl(),
					translate("close"), wc.getInitialComponent());
			listenTo(cmc);
			cmc.activate();
			return;
		}
	...
	public void activate(UserRequest ureq, List<ContextEntry> entries,
		...
		if (CatalogEntry.class.getSimpleName().equals(type)) {
			...
			if (activatedNode != null) {
				...
				activateContent(ureq, "search.catalog", entries,
						entry.getTransientState());
			}
		} else if (RepositoryEntry.class.getSimpleName().equals(type)) {
			Long key = entry.getOLATResourceable().getResourceableId();
			RepositoryEntry selectedEntry = RepositoryManager.getInstance()
					.lookupRepositoryEntry(key);
			if (selectedEntry != null) {
				ToolController toolC = detailsController.setEntry(
						selectedEntry, ureq, true);
				Component toolComp = (toolC == null ? null : toolC
						.getInitialComponent());
				columnsLayoutCtr.setCol2(toolComp);
				mainPanel.setContent(detailsController.getInitialComponent());
			...
			TreeNode rootNode = menuTree.getTreeModel().getRootNode();
			TreeNode activatedNode = TreeHelper.findNodeByUserObject(type,
					rootNode);
			if (activatedNode != null) {
				menuTree.setSelectedNodeId(activatedNode.getIdent());
				List<ContextEntry> subEntries = entries.subList(1,
						entries.size());
				activateContent(ureq, type, subEntries,
						entry.getTransientState());
				if (!subEntries.isEmpty()) {
					ContextEntry nextEntry = subEntries.get(0);
					String subType = nextEntry.getOLATResourceable()
							.getResourceableTypeName();
					if (RepositoryEntry.class.getSimpleName().equals(subType)) {
						searchController.activate(ureq, subEntries,
								nextEntry.getTransientState());
						detailsController.activate(ureq,
								subEntries.subList(1, subEntries.size()),
								nextEntry.getTransientState());
					} else if (CatalogEntry.class.getSimpleName().equals(
							subType)) {
						catalogCntrllr.activate(ureq, subEntries,
								entry.getTransientState());
					}
				}
			}
