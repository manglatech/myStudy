...
public class LoginAuthprovidersController extends MainLayoutBasicController implements Activateable2 {
	...
	private VelocityContainer features;
	private Controller authController;
	private Controller featureDisaplyController;
	...
	public LoginAuthprovidersController(UserRequest ureq, WindowControl wControl) {
		...
		if(!LoginModule.isFeatureLayoutEnabled()){
			// DMZ navigation
			...
			columnLayoutCtr = new LayoutMain3ColsController(ureq, getWindowControl(), null, null, dmzPanel, "useradminmain");
		}
	...
	private VelocityContainer initLoginContent(UserRequest ureq, String provider) {
		...
		if(LoginModule.isFeatureLayoutEnabled()){
			featureDisaplyController = new FeatureDisplayContoller(ureq,getWindowControl());
			contentBorn.put("features", featureDisaplyController.getInitialComponent());
			contentBorn.contextPut("featureEnabled", Boolean.TRUE);
		}
