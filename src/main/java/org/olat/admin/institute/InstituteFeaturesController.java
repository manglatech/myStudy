...
public class InstituteFeaturesController extends BasicController {
	...
	public InstituteFeaturesController(UserRequest ureq, WindowControl wControl) {
		super(ureq, wControl, Util.createPackageTranslator(InstituteFeaturesController.class, ureq.getLocale()));
		...
		loadDataModel(settingsForm.getInstituteList().getSelectedKey());
	...
	private Map<String, String> loadFeatures() {
		List<Institute> f = instituteService.findAllInstitute();
		Map<String,String> map = new HashMap<String, String>();
		for(Institute feature : f){
			map.put(feature.getInstituteId(), feature.getName());
		}
		return map;
	}
	...
	protected void event(UserRequest ureq, Controller source, Event event) {
		if(source == settingsForm){
			if(event.getCommand() == Event.CHANGED_EVENT.getCommand()){
				String key = settingsForm.getInstituteList().getSelectedKey();
				loadDataModel(key);
			}
