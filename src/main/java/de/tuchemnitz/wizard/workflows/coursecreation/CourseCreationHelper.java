...
public class CourseCreationHelper {
	...
	public void finalizeWorkflow(final UserRequest ureq) {
		...
		if (courseConfig.getPublish()) {
			...
			if (courseConfig.getAclType().equals(""+RepositoryEntry.ACC_OWNERS)) {
				// set "BARG" as rule
				access = RepositoryEntry.ACC_OWNERS;
			} else if (courseConfig.getAclType().equals(""+RepositoryEntry.ACC_OWNERS_AUTHORS)) {
				// set "BAR" as rule
				access = RepositoryEntry.ACC_OWNERS_AUTHORS;
			}else if (courseConfig.getAclType().equals(""+RepositoryEntry.ACC_USERS)) {
				// set "BARG" as rule
				access = RepositoryEntry.ACC_USERS;
			} else if (courseConfig.getAclType().equals(RepositoryEntry.MEMBERS_ONLY)) {
				// set "BAR" as rule
				access = RepositoryEntry.ACC_OWNERS;
				memeberOnly = true;
			...
			addedEntry = RepositoryManager.getInstance().setAccess(addedEntry, access, memeberOnly);
		}
